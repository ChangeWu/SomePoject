...
public class CustomViewBehind extends ViewGroup {
	...
	private int mSecondWidthOffset;//副菜单偏移
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (mSecondaryContent != null)
			mSecondaryContent.layout(0, 0, width-mSecondWidthOffset, height);
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mSecondaryContent != null){
			final int contentSecondWidth = getChildMeasureSpec(widthMeasureSpec, 0, width-mSecondWidthOffset);
			mSecondaryContent.measure(contentSecondWidth, contentHeight);
		}
	...
	public void scrollBehindTo(View content, int x, int y) {
		...
		if (mMode == SlidingMenu.LEFT) {
			...
			if (x <= content.getLeft()) {
				...
				scrollTo((int)(getSecondBehindWidth() -getWidth() + 
						(x-getSecondBehindWidth())*mScrollScale), y);				
			}
		}
	...
	public int getMenuLeft(View content, int page) {
		if (mMode == SlidingMenu.LEFT) {
			...
			case 0:
				return content.getLeft() - getBehindWidth();
			case 2:
				return content.getLeft() + getSecondBehindWidth();
			}
		}
		return content.getLeft();
	}
	...
	public void drawFade(View content, Canvas canvas, float openPercent) {
		...
		if (mMode == SlidingMenu.LEFT) {
			...
			left = content.getRight();
			right = content.getRight() + getSecondBehindWidth();			
		}
		canvas.drawRect(left, 0, right, getHeight(), mFadePaint);
	}
