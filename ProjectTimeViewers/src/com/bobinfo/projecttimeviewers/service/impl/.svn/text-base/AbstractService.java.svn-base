...
import com.bobinfo.projecttimeviewers.service.IService;
import com.bobinfo.projecttimeviewers.util.BasicServiceParams;
import com.bobinfo.projecttimeviewers.util.HelperUtil;
 ...
 * @param <SerParam>
 */
public abstract class AbstractService<SerParam extends BasicServiceParams>
		implements IService<SerParam> {
	...
	protected <T> T Send(AbstractRequest<T> request)
		...
		try {
			...
			synchronized (this) {
				mHttpClient = null;
				reset();
				DLog.w(this.getClass().getSimpleName(),
